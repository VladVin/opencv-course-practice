...
int main(int argc, char* argv[])
{
    ...
    {
        KeyPoint objectImgPoint = keypoints1[matches[matchInd].queryIdx];
        KeyPoint sceneImgPoint = keypoints2[matches[matchInd].trainIdx];

        float dist = sqrt((center.x - objectImgPoint.pt.x) * (center.x - objectImgPoint.pt.x) +
            (center.y - objectImgPoint.pt.y) * (center.y - objectImgPoint.pt.y));
        float scaleRatio = sceneImgPoint.size / objectImgPoint.size;
        float angle = atan((center.y - objectImgPoint.pt.y) / (center.x - objectImgPoint.pt.x));
        float angleDelta = M_PI / 180.0f * (sceneImgPoint.angle - objectImgPoint.angle);
        float sceneAngle = angle + angleDelta;
        float sceneDist = scaleRatio * dist;

        Point2f newCenter(sceneImgPoint.pt.x + sceneDist * cos(sceneAngle),
             sceneImgPoint.pt.y + sceneDist * sin(sceneAngle));
        hgt.at<int>(newCenter.y / splitSet, newCenter.x / splitSet)++;
